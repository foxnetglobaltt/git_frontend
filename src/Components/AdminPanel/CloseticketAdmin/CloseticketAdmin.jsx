...
import TicketDetailsModalClose from "./TicketDetailsModalClose";
...
const [isFilterPopupVisible, setIsFilterPopupVisible] = useState(false); // To toggle the visibility of the FilterPopup
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/tickets/closed`);
...
console.log('Total Days to display:', totalDays);
    ...
    generateServiceTicketAdminPDF(ticket.ticketNo, eta, createdDate);
  }
};
